var _ = require('lodash')
  ...
  , log = require('../../logger').getLogger('dodo.core.app.main');
...
function startHttpServer(app, callback) {
  ...
  app.server.listen(app.config.port, app.config.host || void 0, void 0, callback);
}
...
function logStartup(app) {
  ...
  if (!testing) {
    try {
      var packageJsonLocation = config.serviceRoot || '.';
      packageJson = require( packageJsonLocation + '/package.json');
    } catch(err) {
      packageJson = {name: 'unknown', version: 'unknown'};
    }
    log.info(color.white('Running ')
